...
test('настройки открываются кнопкой open-settings и содержат план/секрет/звук (недели тут НЕТ)', async ({ page }) => {
  await open(page)
  ...
  // Поле секрета (по умолчанию password — селектим по aria-label, не по role).
  await expect(settings(page).getByLabel('секрет записи')).toBeVisible()
...
test('селект недели — в ШАПКЕ, а не в настройках; смена недели меняет веса (W1=120, W2=125)', async ({ page }) => {
  ...
  // В настройках недели НЕТ (план/секрет/звук остались).
  await openSettings(page)
  await expect(settings(page).getByRole('combobox', { name: 'неделя' })).toHaveCount(0)
  await expect(settings(page).getByRole('combobox', { name: 'план' })).toBeVisible()
})

test('секрет: сохранение значения переживает переоткрытие настроек', async ({ page }) => {
  await open(page)
  await openSettings(page)
  // Показать секрет (по умолчанию password) — поле под тем же aria-label.
  await settings(page).getByRole('button', { name: 'показать секрет' }).click()
  const secret = settings(page).getByLabel('секрет записи')
  await secret.fill('s3cr3t')
  await settings(page).getByRole('button', { name: 'Сохранить' }).click()
  await expect(settings(page)).toHaveCount(0)

  // Переоткрыть — значение сохранилось в localStorage.
  await openSettings(page)
  await settings(page).getByRole('button', { name: 'показать секрет' }).click()
  await expect(settings(page).getByLabel('секрет записи')).toHaveValue('s3cr3t')
})
